  ...
  class BasicCharacterController extends entity.Component {
    ...
    _Init(params) {
      ...
      this._stateMachine = new CharacterFSM(
        ...
        // Create and add the BoxHelper for the bounding box visualization
      //this._boundingBoxHelper = new THREE.BoxHelper(new THREE.Object3D(), 0xffff00); // Yellow color for visibility
      //this._params.scene.add(this._boundingBoxHelper); // Add helper to the scene
    ...
    _OnDeath(msg) {
      ...
      setTimeout(() => {
        window.location.href = 'player-death2.html'; // Redirect to the new page
    ...
    _LoadModels() {
      ...
      loader.load('castle_guard_01.fbx', (fbx) => {
        ...
        //this._parent
        //console.table(this._parent._position);
        ///const center = this._parent._position;
        ...
        const offset = new THREE.Vector3(1, 1, 1); // Adjust these values as needed for each axis

        // Shrink the bounding box by applying the offset
        //this._boundingBox.min.add(offset);
        //this._boundingBox.max.sub(offset);
        this._boundingBox.expandByScalar(-2);

        // Set the helper to reflect the bounding box of the player model
        //this._boundingBoxHelper.setFromObject(this._target);
    ...
    Update(timeInSeconds) {
      ...
      this._boundingBox.setFromObject(this._target);
      this._boundingBox.expandByScalar(-2);
      // Update the BoxHelper to align with the updated bounding box
      //this._boundingBoxHelper.setFromObject(this._target);
